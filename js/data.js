/* ============================================================
   data.js — all course content (ES5)
   ...
   Each fable: vocab warm-up, illustrated story, then the four
   sections: LŪDUS (game), AENIGMATA (puzzles), CORRIGE
   (correct the mistakes), COMPLĒ (complete the sentences).
   LANGUAGE-AGNOSTIC: nothing in the engine reads Latin; adding
   English/Spanish later = adding objects to DATA.fables.
   ============================================================ */
var DATA = (function () {
  'use strict';
  ...
  /* ---------- UI strings (kept as data: swap per language later) ---------- */
  ...
  /* seeded leaderboard bots (server-side once PHP lands) */
  var BOTS = [
    { name: 'Marcus',  xp: 340 },
    { name: 'Iūlia',   xp: 285 },
    { name: 'Aemilia', xp: 210 },
  ...
  var STEP_ICONS = { verba: '🖼️', fabula: '📜', ludus: '🕹️', aenigmata: '🧩', corrige: '✏️', comple: '✍️' };

  /* ---------- overworld map: the "Provincia" bonus path ----------
     A region groups spine fables and ends in a boss castle. The boss is a
     timed fight (Boss module) followed by a cumulative quiz. Beating both
     clears the region. v1 = one region (the 3 starter fables) + a locked
     future region beyond the gate. Node layout is in MAP grid units; the
     map renderer turns these into an SMB3-style overworld. */
  var REGIONS = [
    {
      id: 'region1',
      titulus: 'Prōvincia Prīma',
      fables: ['f1', 'f2', 'f3'],
      boss: {
        ...
        /* quiz: 5 cumulative questions, word → pick the image. Pulled across
           all three fables (one each from f1/f2/f3 + two extra). Indices map
           into each fable's vocab by `la`. */
        quiz: [
          { la: 'vulpēs', from: 'f1' },
          { la: 'cāseus', from: 'f2' },
          { la: 'lupus',  from: 'f3' },
  ...
  /* Map node layout. x,y are fractions (0..1) of the scrollable map canvas,
     so the overworld scales to any width. `kind`: 'fable' nodes open the
     spine fable; 'boss' opens the castle; 'gate' is the locked future entry.
     `links` lists node ids this node connects to (for drawing the path). */
  var MAP = {
    bg: 'overworld',
    nodes: [
      { id: 'f1',    kind: 'fable', region: 'region1', fable: 'f1', x: 0.22, y: 0.86, label: 'I',  links: ['f2'] },
      { id: 'f2',    kind: 'fable', region: 'region1', fable: 'f2', x: 0.50, y: 0.66, label: 'II', links: ['f3'] },
      { id: 'f3',    kind: 'fable', region: 'region1', fable: 'f3', x: 0.30, y: 0.46, label: 'III', links: ['boss1'] },
      { id: 'boss1', kind: 'boss',  region: 'region1', x: 0.62, y: 0.28, label: '👑', links: ['gate1'] },
      { id: 'gate1', kind: 'gate',  region: 'region2', x: 0.50, y: 0.10, label: '🔒', links: [] }
    ]
  };

  var MAP_UI = {
    titulus: 'Prōvincia',
    bossLocked: 'Perfice omnēs fābulās prīmum!', /* finish all fables first */
    bossReady: 'Lupum vince!',                    /* defeat the wolf */
    pugna: 'PUGNA',                               /* fight */
    quizTitle: 'Probātiō',                        /* the test */
    gateLocked: 'Mox plūra…',                     /* more soon */
    vicisti: 'VĪCISTĪ!',                          /* you won */
    victus: 'Iterum tempta!',                     /* try again */
    tempus: 'Tempus',                             /* time */
    cursus: 'Cursus',                             /* the lesson spine */
    provincia: 'Prōvincia'
  };

  return {
    fables: fables, UI: UI, BOTS: BOTS, STEPS: STEPS, STEP_ICONS: STEP_ICONS,
    REGIONS: REGIONS, MAP: MAP, MAP_UI: MAP_UI
  };
})();
